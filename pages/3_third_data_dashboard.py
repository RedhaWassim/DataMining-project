...
# Load data
df = pd.read_excel(r"/home/redha/Documents/projects/NLP/streamlit test/streamlit_test/data/Dataset3.xlsx")

st.sidebar.header('User Input Features')
selected_histogram = st.sidebar.selectbox('Histogram Variable', df.columns[:-1])
selected_pie = st.sidebar.selectbox('Pie Chart Category', ['Soil', 'Crop', 'Fertilizer'])
selected_scatter_x = st.sidebar.selectbox('X-axis for Scatter Plot', df.columns[:-3], index=0)
selected_scatter_y = st.sidebar.selectbox('Y-axis for Scatter Plot', df.columns[:-3], index=1)
...
fig_scatter_matrix = px.scatter_matrix(df, dimensions=['Temperature', 'Humidity', 'Rainfall'], color='Crop')
...
treemap_option = st.sidebar.selectbox('Select the category for treemap:',
                                      options=['Crop', 'Soil'])
