...
# Load data
df = pd.read_csv(
    r"/home/redha/Documents/projects/NLP/streamlit test/streamlit_test/data/second_df.csv"
)
...
fig_tree = px.treemap(
    ...
    path=["zcta", "Year"],
    values="case count",
    title="Confirmed Cases by Zone and Year",
)
...
agg_data_by_zone_year = df.groupby(["zcta", "Year"], as_index=False)[
    "positive tests"
].sum()
...
fig_bubble.update_layout(
    coloraxis_showscale=False, legend_title_text="Zone"  # Hide the color scale
)
...
selected_zone = st.sidebar.selectbox(
    "Select a zone for time series analysis", df["zcta"].unique()
)
...
weekly_data = (
    zone_data.groupby("Week")
    .agg({"test count": "sum", "positive tests": "sum", "case count": "sum"})
    .reset_index()
)
monthly_data = (
    zone_data.groupby("Month")
    .agg({"test count": "sum", "positive tests": "sum", "case count": "sum"})
    .reset_index()
)
annual_data = (
    zone_data.groupby("Year")
    .agg({"test count": "sum", "positive tests": "sum", "case count": "sum"})
    .reset_index()
)
...
def create_line_chart(data, x_axis, title):
    fig = px.line(
        data,
        x=x_axis,
        y=["test count", "positive tests", "case count"],
        markers=True,
        title=title,
    )
    return fig
